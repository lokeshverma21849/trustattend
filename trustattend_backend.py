"""
TrustAttend — Cloud-Ready Production Backend  v3.1.0
FastAPI + PostgreSQL (Supabase/Render) + SQLAlchemy + bcrypt + SHA-256 Integrity
=================================================================================
NEW in v3.1:
  - Migrated persistence from local SQLite to PostgreSQL via DATABASE_URL env var
  - Auto-fixes "postgres://" → "postgresql://" for Render/Heroku-style URLs
  - pool_pre_ping=True to survive idle-connection drops on free-tier Postgres
  - Falls back to local SQLite automatically if DATABASE_URL is not set (dev only)
...
Local run (SQLite fallback, no DATABASE_URL needed):
    pip install -r requirements.txt
    python trustattend_backend.py

Production run (Supabase/Render Postgres):
    1. Create a free Postgres database on Supabase (or use Render's free Postgres).
    2. Copy the connection string (Session pooler / "Connection string" in Supabase
       Settings → Database). It looks like:
       postgresql://postgres:[PASSWORD]@db.xxxxxxxx.supabase.co:5432/postgres
    3. Set it as an environment variable named DATABASE_URL on Render
...
# DATABASE_URL is read from the environment (e.g. Supabase / Render Postgres
# connection string). Falls back to a local SQLite file only when no
# DATABASE_URL is set, so local development still works without Postgres.
DATABASE_URL = os.environ.get("DATABASE_URL")

# Render/Heroku-style Postgres URLs sometimes start with "postgres://" but
# SQLAlchemy (via psycopg2) requires the "postgresql://" scheme.
...
if not DATABASE_URL:
    # Local fallback — ephemeral SQLite file, only used when DATABASE_URL is unset.
    _db_path = os.environ.get("DB_PATH", os.path.join(os.path.dirname(os.path.abspath(__file__)), "trustattend.db"))
    DATABASE_URL = f"sqlite:///{_db_path}"
    os.makedirs(os.path.dirname(_db_path), exist_ok=True)
else:
    _db_path = None  # not used when running on Postgres

# ══════════════════════════════════════════════════════════════
#  DATABASE SETUP
# ══════════════════════════════════════════════════════════════
if _IS_POSTGRES:
    # PostgreSQL does not use / support SQLite's connect_args.
    # pool_pre_ping avoids "server closed the connection unexpectedly"
    # errors after Supabase/Render free-tier connections idle out.
...
def startup():
    ...
    if _IS_POSTGRES:
        # Hide credentials in logs — only show host/db, never the password.
        _safe_url = DATABASE_URL.split("@")[-1] if "@" in DATABASE_URL else DATABASE_URL
        print(f"📂 Database : PostgreSQL @ {_safe_url}")
    else:
        print(f"📂 Database : SQLite (local, ephemeral) — {_db_path}")
...
def health():
    ...
    return {
        ...
        "db_path": _db_path if not _IS_POSTGRES else None,
        "cors_origin": ALLOWED_ORIGIN,
        "features": ["multi-role", "duty-types", "gps-checkin", "travel-status", "postgres-persistence"],
    }
